// Package tldlookup provides a Go client for the TLD Lookup API.
//
// For more information, visit: https://apiverve.com/marketplace/tldlookup?utm_source=go&utm_medium=readme
package tldlookup
...
const (
	baseURL        = "https://api.apiverve.com/v1/tldlookup"
	defaultTimeout = 30 * time.Second
)

// Client is the TLD Lookup API client.
type Client struct {
	apiKey     string
	httpClient *resty.Client
}

// NewClient creates a new TLD Lookup API client.
...
// Execute makes a request to the TLD Lookup API with typed parameters.
//
...
//   - tld (required): string - The top-level domain to look up (with or without leading dot)
